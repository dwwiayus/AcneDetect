...
# Bobot scoring (harus konsisten dengan notebook)
WEIGHTS = {
    "cosine" : 0.35,
    "skor"   : 0.35,
    "kulit"  : 0.20,
...
def run_recommendation(
    ...
    """Logika rekomendasi — konsisten dengan notebook skincare_recommendation.ipynb"""
    df      = model_bundle["df"]
    ...
    cos_sim   = cosine_similarity(query_vec, matrix).flatten()
    ...
    bonus_bahan  = (df["Tipe_Bahan_Aktif_Final"].str.lower() == (bahan_preferred or "").lower()).astype(float) * 0.3 if bahan_preferred else 0
    bonus_produk = (df["Jenis_Produk_Final"].str.lower()     == (jenis_produk or "").lower()).astype(float) * 0.2    if jenis_produk    else 0
    rule_score   = np.clip(mask_masalah * 0.5 + mask_kulit * 0.5 + bonus_bahan + bonus_produk, 0, 1)
    ...
    if jenis_produk:
        filtered = result[result["Jenis_Produk_Final"].str.lower() == jenis_produk.lower()]
...
def recommend(req: RecommendRequest):
    """
    Rekomendasi produk skincare berdasarkan masalah dan jenis kulit.
